...
CREATE TABLE public.store_profile (
    ...
    tax_type text DEFAULT 'exclusive'::text NOT NULL,
    service_charge_percentage integer DEFAULT 0 NOT NULL,
    receipt_footer text,
    receipt_header text,
    loyalty_point_conversion integer DEFAULT 10000 NOT NULL,
    loyalty_point_value integer DEFAULT 1 NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    is_dirty boolean DEFAULT false NOT NULL,
    deleted_at timestamp with time zone,
    deduct_stock_on_hold boolean DEFAULT false NOT NULL,
    user_id uuid,
    business_type text
);
