...
type Repository interface {
	...
	FindByEmail(email string) (*models.License, error)
	FindByUserID(userID string) (*models.License, error)
...
func (r *repository) FindByEmail(email string) (*models.License, error) {
	var license models.License
	err := r.db.Preload("Devices").Where("customer_email = ?", email).First(&license).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil // Return nil, nil for not found to handle explicitly in service
		}
...
func (r *repository) FindByUserID(userID string) (*models.License, error) {
	var license models.License
	err := r.db.Preload("Devices").Where("user_id = ?", userID).First(&license).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
